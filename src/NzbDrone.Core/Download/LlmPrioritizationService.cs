    ...
    public class LlmPrioritizationService : ILlmPrioritizationService
    {
        ...
        public async Task<List<DownloadDecision>> ApplyAsync(List<DownloadDecision> sorted)
        {
            ...
            try
            {
                ...
                var prompt = BuildPrompt(movie, sorted);
                ...
                if (_cache.TryGetValue(cacheKey, out var cached) && cached.expiry > DateTime.UtcNow)
                {
                    ...
                    {
                        _logger.Info("LLM (cached) declined all releases for '{0}', skipping download", movie.Title);
                ...
                var choice = await TryProvidersAsync(urls, apiKeys, models, systemPrompt, prompt, sorted.Count, movie.Title, timeout, maxTokens, temperature);
                ...
                if (choice.Value == 0)
                {
                    _logger.Info("LLM declined all releases (choice: 0) for '{0}', skipping download", movie.Title);
                    return new List<DownloadDecision>();
                }

                var selected = sorted[choice.Value - 1];
                _logger.Info("LLM selected release #{0}: {1}", choice.Value, selected.RemoteMovie.Release.Title);
