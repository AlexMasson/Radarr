...
namespace NzbDrone.Core.Download
{
    ...
    public class LlmPrioritizationService : ILlmPrioritizationService
    {
        ...
        public async Task<List<DownloadDecision>> ApplyAsync(List<DownloadDecision> sorted)
        {
            ...
            // RSS sync sends decisions for ALL movies in one batch. Group them by movie so
            // each LLM call has a coherent prompt (one movie + its competing releases) and
            // cache keys are per-movie — an unchanged release list for a movie = cache hit
            // = no API call, regardless of what's happening for other movies in the batch.
            var groupOrder = new List<int>();
            var groups = new Dictionary<int, List<DownloadDecision>>();

            foreach (var d in sorted)
            {
                var key = d.RemoteMovie.Movie.Id;
            ...
            double temperature)
        {
            ...
            var movie = firstDecision.RemoteMovie.Movie;

            try
            {
                var systemPrompt = GetSystemPromptForProfile(movie.QualityProfileId);
                ...
                var prompt = BuildPrompt(movie, group);
                var cacheKey = ComputeCacheKey(systemPrompt, prompt);

                if (_cache.TryGetValue(cacheKey, out var cached) && cached.expiry > DateTime.UtcNow)
                {
                    _logger.Debug("LLM cache hit for '{0}', reusing choice", movie.Title);
                    return ApplyChoice(group, cached.choice, movie.Title, fromCache: true);
                }

                _logger.Debug("LLM dispatching to {0} provider(s) for '{1}' (system prompt: {2} chars, user prompt: {3} chars)",
                    urls.Count, movie.Title, systemPrompt.Length, prompt.Length);
                _logger.Debug("LLM user prompt:\n{0}", prompt);

                var choice = await TryProvidersAsync(urls, apiKeys, models, systemPrompt, prompt, group.Count, movie.Title, timeout, maxTokens, temperature);

                if (!choice.HasValue)
                {
                    _logger.Warn("LLM all {0} provider(s) failed or returned unparseable responses for '{1}', using default priority", urls.Count, movie.Title);
                    return group;
                }

                _cache[cacheKey] = (choice, DateTime.UtcNow.AddMinutes(60));

                return ApplyChoice(group, choice, movie.Title, fromCache: false);
            }
            catch (Exception ex)
            {
                _logger.Warn(ex, "LLM prioritization failed for '{0}', using default priority", movie.Title);
                return group;
            }
        }
        ...
        private List<DownloadDecision> ApplyChoice(List<DownloadDecision> group, int? choice, string itemTitle, bool fromCache)
        {
            ...
            if (!fromCache)
            {
                _logger.Info("LLM selected release #{0} for '{1}': {2}", choice.Value, itemTitle, selected.RemoteMovie.Release.Title);
            }
        ...
        private string BuildPrompt(Movies.Movie movie, List<DownloadDecision> decisions)
        {
            var sb = new StringBuilder();
            sb.AppendLine($"Movie: {movie.Title} ({movie.Year})");
            sb.AppendLine();
            ...
            {
                var d = decisions[i];
                var remote = d.RemoteMovie;
                var release = remote.Release;
                var quality = remote.ParsedMovieInfo?.Quality?.Quality?.Name ?? "Unknown";
