        ...
        public async Task<List<DownloadDecision>> ApplyAsync(List<DownloadDecision> sorted)
        {
            ...
            try
            {
                ...
                var payload = new
                {
                    ...
                    messages = new[]
                    {
                        new { role = "system", content = "You are a release selection assistant for Radarr. Given a list of releases, pick the best one. Consider: quality, size, seeders, codec, release group reputation, language, custom format score. Return ONLY: {\"choice\": <1-based index>}" },
